...
@endphp
    ...
    <ul class="pagination justify-content-center mb-0 py-2">
        ...
        @endphp
        <li class="page-item "><a class="page-link " href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $searchPrev['skip']])  }}"><i
        ...
        <li class="page-item">
            ...
            @endphp
            <a class="page-link" href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $previous['skip']] ) }}" @if ($skip < 1 ) hidden @endif
                tabindex="-1">
        ...
        @if ($limit > 11)
        <li class="page-item "><a class="page-link " href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $searchReset['skip']])  }}">{{1}}</a>
        </li>...
        @endif
        @for ($position; $position < ($limit); $position++) <div>
            ...
            <li class="page-item @if(request()->input('skip') == $position) active @endif">
                <a class="page-link " href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $search[$position]['skip']])  }}">{{$position + 1}}</a>
            </li>
            @endif
            @endfor
            ...
            @if ($limit < $paginate) ... @php $searchLast=request()->input();
                ...
                @endphp <li class="page-item "><a class="page-link "
                        href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $searchLast['skip']])  }}">{{$paginate}}</a>
                </li>
                @endif
                ...
                <li class="page-item">
                    <a class="page-link" href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $next['skip']] ) }}">
                ...
                @if ($paginate > 10 && ($skip + 10) < $paginate) @php $searchNext=request()->input();
                    ...
                    <li class="page-item "><a class="page-link "
                            href="{{ route("$optionsRoutes.show", ['data' => $data, 'skip' => $searchNext['skip']])  }}">
