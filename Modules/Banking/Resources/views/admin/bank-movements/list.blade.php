    ...
    <div class="card">
        ...
        <div class="table">
            <table class="table-striped table align-items-center table-flush table-hover">
                ...
                <tbody>
                    @foreach($bankMovements as $data)
                    <tr>
                        <td class="text-center">{{ $data->created_at->format('M d, Y h:i a') }}</td>
                        <td class="text-center">$ {{ number_format($data->amount, 2) }}</td>
                        <td class="text-center"> {{ $data->movement_type}}</td>
                        <td class="text-center"> {{ $data->bankAccount->name }} / 
                            {{ $data->bankAccount->bank->name }}
                        </td>
                        <td class="text-center">$ {{ number_format($data->total_bank_amount, 2)}} {{$data->bankAccount->bank->country->exchange_code}}</td>
                    </tr>
                        ...
                        <div class="modal-dialog modal-sm" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">Actualizar movimiento de banco <b>{{$data->name}}</b></h5>
        ...
        <div class="card-footer py-2 text-center">
            {{ $bankMovements->appends(request()->query())->links() }}
        </div>
    </div>
    @else
